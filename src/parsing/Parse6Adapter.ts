 ...
 */
export class Parse6Adapter implements CSSSelectAdapter<Node, Element> {
    ...
    existsOne(test: Predicate<Element>, elems: Node[]): boolean {
        return elems.some((value, index, array) => {
    ...
    _isParent(node: any) : node is ParentNode {
...
/**
 * This is an instance of the `Parse6Adapter` class which uses the
 * `defaultTreeAdapter` from `parse5` to manipulate the AST.  This
 * is most likely what you'll want to import.
 */
export const parse6Adapter = new Parse6Adapter(defaultTreeAdapter);